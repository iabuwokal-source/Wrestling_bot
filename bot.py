...
# ───────────────────────────────────────────────
# المفاتيح من متغيرات البيئة (أكثر أماناً)
# ───────────────────────────────────────────────
...
async def news_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """جلب ونشر خبر فوري عند الطلب"""

    await update.message.reply_text(
        "⏳ جاري جلب آخر خبر من أخبار جوجل..."
    )
    ...
    if not news_item:
        await update.message.reply_text(
            "❌ لم أتمكن من جلب الأخبار حالياً، جرب لاحقاً."
        )
        return

    prompt = f"""
أنت محرر صحفي رياضي خبير ومختص في رصد أخبار جميع اتحادات المصارعة الحرة في العالم
(سواء كانت WWE بأقسامها Raw, SmackDown, NXT أو AEW بقسميها Dynamite, Collision
أو الاتحادات اليابانية NJPW, Stardom أو TNA, ROH، والاتحادات المكسيكية AAA, CMLL،
والمستقلة).
...
🏷️ **التصنيف:** [عاجل / تقرير] | **اتحاد:** [اكتب اسم الاتحاد الصحيح بدقة، مثل #WWE أو #AEW أو #NJPW أو #TNA وغيرها]
"""
...
async def fetch_specific_news(
    ...
    await update.message.reply_text(
        f"⏳ جاري البحث عن أخبار {federation}..."
    )

    rss_url = (
        f"https://news.google.com/rss/search?"
        f"q={federation}+wrestling+news"
    ...
    try:
        ...
        if not feed.entries:
            await update.message.reply_text(
                f"❌ لم أجد أخبار {federation} حالياً."
            )
            return
...
# ───────────────────────────────────────────────
# المراسلة العادية (نص، صور، فيديو، صوت)
# ───────────────────────────────────────────────
...
async def handle_user_input(
    ...
    # إذا أرسل نص بدون وسائط → تحقق من كونه طلباً
    if (
        not message.photo
    ...
    if not caption_text:
        caption_text = (
            "تغطية خاصة لأبرز مستجدات المصارعة الحرة."
        )

    # صياغة المحتوى
    prompt = f"""
أنت محرر صحفي رياضي محترف.
قم بصياغة المحتوى التالي ليظهر كـ "خبر مستقل ومنظم":
...
"""
    ...
    try:
        ...
        await message.reply_text(
            "✅ تمت الصياغة والنشر في القناة بنجاح!"
        )

    except Exception as e:

        logger.error(
            f"خطأ في معالجة الرسالة: {e}"
        )
    ...
    while True:
        ...
        try:
            ...
            if news_item:
                ...
                # تجنب نشر نفس الخبر مرتين
                if news_item['title'] == last_published_title:

                    logger.info(
                        "الخبر نفسه تم نشره مسبقاً، سيتم التخطي."
                    )
...
"""
                    ...
                    logger.info(
                        "تم جلب ونشر الخبر التلقائي لكافة الاتحادات بنجاح."
                    )

            else:

                logger.warning(
                    "⚠️ لم يتم العثور على أخبار جديدة."
                )

        except Exception as e:

            logger.error(
                f"خطأ في حلقة النشر التلقائي: {e}"
            )
...
async def post_init(application):
    ...
    asyncio.create_task(
        automated_news_loop(application)
    )
...
def main():
    ...
    application.add_handler(
        CommandHandler("wwe", wwe_command)
    )

    application.add_handler(
        CommandHandler("aew", aew_command)
    )

    application.add_handler(
        CommandHandler("njpw", njpw_command)
    )

    application.add_handler(
        CommandHandler("tna", tna_command)
    )
